...
(function () {
		...
		if (plugins.preloader.length && !isNoviBuilder) {
			pageTransition({
				...
				onTransitionStart: function (options) {
					// Eliminar el preloader instantáneamente
					plugins.preloader.addClass('loaded');
				},
				onReady: function () {
					// Mantén el preloader oculto desde el principio
					plugins.preloader.addClass('loaded');
					windowReady = true;
				}
			});
		...
		if (plugins.countdown.length) {
			for (let i = 0; i < plugins.countdown.length; i++) {
				...
				if ($countDownItem.attr('data-style') === 'short') {
					settings['labels'] = ['Años', 'Meses', 'Semanas', 'Días', 'Hrs', 'Min', 'Seg'];
				}
	...
	$(function () {
		...
		function attachFormValidator(elements) {
			// Validador personalizado para número de teléfono (puedes eliminarlo si no lo necesitas)
			regula.custom({
				name: 'PhoneNumber',
			...
			// Iterar sobre los elementos del formulario y añadir la validación
			...
			// Validar en diferentes eventos (input, blur, etc.)
			elements.on('input change propertychange blur', function (e) {
				var $this = $(this), results;
		
				// Evitar mostrar errores si el usuario aún está completando el campo
			...
			var regularConstraintsMessages = [
				...
				},
				{
					...
					newMessage: "Sólo se permiten números."
				},
				{
			...
			// Aplicar los mensajes personalizados a las validaciones
    ...
    attachFormValidator($('#loginForm [data-constraints]'));
    ...
    $('#loginForm').on('submit', function (event) {
        ...
        // Validar email utilizando Regula
        if (!validateEmail(email)) {
            errores.push("El Email es obligatorio o no es válido");
        }
        if (!password) {
        ...
        if (errores.length > 0) {
            $('#alerta').html('<div class="alert alert-danger">' + errores.join('<br>') + '</div>');
            return false; // No envía el formulario
        }

        // Si no hay errores, proceder a enviar el formulario por AJAX
        $.ajax({
            url: 'login.php', // Cambia esto al archivo PHP que procesará el login
            type: 'POST',
            data: {
                email: email,
                password: password
            ...
            success: function (response) {
                if (response === 'success') {
                    ...
                    $('#alerta').html('<div class="alert alert-success">Login exitoso</div>');
                    window.location.href = '/admin/index.php'; // Redirigir al área protegida
                } else {
                    // Mostrar error si el login falla
                    $('#alerta').html('<div class="alert alert-danger">' + response + '</div>');
                }
            },
            error: function () {
                $('#alerta').html('<div class="alert alert-danger">Error en el servidor. Inténtalo más tarde.</div>');
            }
        });
    });
    ...
    // Función para adjuntar validador de formulario usando Regula
    function attachFormValidator(elements) {
        regula.custom({
            ...
            validator: function () {
                if (this.value === '') return true;
                else return validateEmail(this.value);
            }
        });
        ...
        elements.on('input change propertychange blur', function (e) {
            ...
            if ((results = $this.regula('validate')).length) {
                for (var i = 0; i < results.length; i++) {
                    $('#alerta').html('<div class="alert alert-danger">' + results[i].message + '</div>');
                }
            } else {
                $('#alerta').html(''); // Limpiar alertas
            }
        }).regula('bind');
    }
});
...
$(document).ready(function () {
	...
	// Manejo del formulario para crear o actualizar usuarios
    $('#crearUsuario').on('submit', function (event) {
        ...
        var action = $('input[type=submit]').val(); // Obtener el valor del botón de submit (Registrar o Actualizar)
        var errores = [];
        ...
        // Validar contraseñas en caso de creación o actualización
        if (action === 'Registrar' && (!password || password !== password2)) {
            errores.push("Las contraseñas deben coincidir y no pueden estar vacías");
        }
        if (action === 'Actualizar' && (currentPassword && !newPassword)) {
            errores.push("Si cambias la contraseña, debes ingresar la nueva contraseña");
        }
        ...
        // Enviar formulario por AJAX si no hay errores
        $.ajax({
            url: form.attr('crear.php'), // URL al que se enviará el formulario
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            ...
            success: function (response) {
                if (response.success) {
                    // Mostrar mensaje de éxito dependiendo de la acción
                    Swal.fire({
                        icon: 'success',
                        title: 'Éxito',
                        text: action === 'Registrar' ? 'Usuario creado exitosamente' : 'Usuario actualizado exitosamente',
                        confirmButtonText: 'Aceptar'
                    }).then(() => {
                        // Redirigir tras éxito
                        window.location.href = '/admin/usuarios/usuarios.php';
                    });
                } else {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: response.error || 'Hubo un problema al procesar los datos.',
                        confirmButtonText: 'Aceptar'
                    });
                }
            },
...
function eliminarUsuario(userId) {
    $.ajax({
        url: 'eliminar.php', // Archivo PHP que manejará la eliminación
        type: 'POST',
        data: { id: userId },
        success: function(response) {
            // Asegúrate de que la respuesta se analice como JSON
            ...
            if (data.success) {
                ...
                $(`.product-cart-delete[data-user-id="${userId}"]`).closest('tr').remove();
            } else {
                Swal.fire(
                    'Error!',
...
$(document).on('click', '#eliminarEquipo', function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            // Llamar a la función para eliminar el usuario
            eliminarEquipo(userId);
        }
    });
});

function eliminarEquipo(userId) {
    $.ajax({
        url: 'eliminar.php', // Archivo PHP que manejará la eliminación
        type: 'POST',
        data: { id: userId },
        success: function(response) {
            ...
            if (data.success) {
                ...
                Swal.fire(
                    'Error!',
                    data.message || 'No se pudo eliminar el equipo.',
                    'error'
                );
            }
        },
